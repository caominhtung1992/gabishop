	...
	class Product extends Student{
		...
		public function add(){
			...
			if($this->input->post('ok') != ""){
				$url = str_replace(' ', '-',strtolower($this->string->makeTitle($this->input->post('pro_name'))));
				$val = array(
		...
		public function update_all(){
			...
			if($this->input->post('pro_new') == 1){
				$val['pro_new_check'] = strtotime("now");
			}
			if($this->input->post('pro_hot') == 1){
				$val['pro_hot_check'] = strtotime("now");
			}
			if($this->input->post('pro_bestsale') == 1){
				$val['pro_bestsale_check'] = strtotime("now");
			}
			if($this->input->post('pro_saleoff') == 1){
				$val['pro_saleoff_check'] = strtotime("now");
			}
			//$this->debug($val);
			$this->mproduct->update_product($val,$id);
		}
